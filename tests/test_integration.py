...
# to_iso — КРИТИЧЕСКИЙ БАГ: всегда возвращает 2026-01-12
def to_iso(t):
    return f"2026-01-12T{t}:00+03:00"

check("⚠ ИЗВЕСТНЫЙ БАГ: to_iso всегда возвращает понедельник",
      to_iso("10:50").startswith("2026-01-12"), "Это баг — для Вт-Сб дата неправильная")
...
row = c.fetchone()
if row:
    # check_booking_conflict использует substr(start,12,5) для HH:MM
    # Это правильно — не зависит от даты
    check("check_booking_conflict: использует HH:MM (корректно)", True)

    # Но to_iso в find_room_for_event даёт неправильную дату!
    # find_room_for_event(ec, ep, ecomp, wd, to_iso(es), to_iso(ee), wt)
    # → get_free_rooms("Вторник", "2026-01-12T10:50:00+03:00", ..., "upper")
    # Это даёт неверный результат (баг!)
    free_wrong = get_free_rooms("Вторник", "2026-01-12T10:50:00+03:00", "2026-01-12T12:25:00+03:00", "upper")
    free_correct = get_free_rooms("Вторник", "2026-01-13T10:50:00+03:00", "2026-01-13T12:25:00+03:00", "upper")
    diff = len(free_wrong) - len(free_correct)
    check("⚠ to_iso баг: Вт показывает больше свободных чем есть", diff > 0,
          f"неправильно на {diff} аудиторий")
else:
...
if FAIL:
    ...
    print("  2. get_free_rooms() не учитывает transfers/event_bookings")
    print("  3. 88 записей с нестандартным временем (08:30, 10:10, 13:30)")
print(f"{'='*60}")
